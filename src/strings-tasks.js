...
function isPalindrome(str) {
  const result = str.replace(/\W/g, '').toLowerCase();
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const result = char <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    return String.fromCharCode(
      ((char.charCodeAt(0) - result + 13) % 26) + result
    );
  });
}
...
function getCardId(value) {
  ...
  const position = views.indexOf(value.charAt(value.length - 1));
  const index = cards.indexOf(value.slice(0, -1));
  return position * cards.length + index;
}
